...
CREATE TABLE "note_relationships" (
    ...
    "strength_score" DECIMAL(3,2) NOT NULL DEFAULT 0.50,
...
-- AddVectorIndex
CREATE INDEX "idx_content_vector_hnsw" ON "notes" USING hnsw ("content_vector" vector_cosine_ops);
...
-- AddCheckConstraints
ALTER TABLE "notes" ADD CONSTRAINT "check_title_not_empty" CHECK (LENGTH(TRIM(title)) > 0);

-- AddCheckConstraints
ALTER TABLE "notes" ADD CONSTRAINT "check_content_not_empty" CHECK (LENGTH(TRIM(content)) > 0);

-- AddCheckConstraints
ALTER TABLE "notes" ADD CONSTRAINT "check_content_hash_not_empty" CHECK (LENGTH(TRIM(content_hash)) > 0);

-- AddCheckConstraints
ALTER TABLE "notes" ADD CONSTRAINT "check_version_positive" CHECK (version > 0);

-- AddCheckConstraints
ALTER TABLE "notes" ADD CONSTRAINT "check_view_count_positive" CHECK (view_count >= 0);

-- AddCheckConstraints
ALTER TABLE "note_relationships" ADD CONSTRAINT "check_strength_score_range" CHECK (strength_score >= 0.0 AND strength_score <= 1.0);

-- AddCheckConstraints
ALTER TABLE "user_feedback" ADD CONSTRAINT "check_rating_range" CHECK (rating >= 1 AND rating <= 5);

-- AddCheckConstraints
ALTER TABLE "ai_processing_logs" ADD CONSTRAINT "check_processing_time_positive" CHECK (processing_time_ms >= 0);

-- AddCheckConstraints
ALTER TABLE "ai_processing_logs" ADD CONSTRAINT "check_input_tokens_positive" CHECK (input_tokens >= 0);

-- AddCheckConstraints
ALTER TABLE "ai_processing_logs" ADD CONSTRAINT "check_output_tokens_positive" CHECK (output_tokens >= 0);

-- AddCheckConstraints
ALTER TABLE "ai_processing_logs" ADD CONSTRAINT "check_cost_positive" CHECK (cost >= 0);

-- AddFullTextSearchIndex
CREATE INDEX "notes_fulltext_search" ON "notes" USING gin(to_tsvector('english', title || ' ' || content));

-- AddFullTextSearchIndex
CREATE INDEX "search_index_content_fts" ON "search_index" USING gin(to_tsvector('english', content));