...
describe('Development Environment Health Check', () => {
  ...
  describe('API Health Endpoint', () => {
    ...
    it('should check Redis connection', async () => {
      const response = await request(app).get('/api/health/redis').expect(200);

      expect(response.body).toHaveProperty('status', 'healthy');
    });

    it('should check AI services availability', async () => {
      const response = await request(app).get('/api/health/ai').expect(200);
  ...
  describe('Environment Validation', () => {
    ...
    it('should check development environment setup', async () => {
      const response = await request(app).get('/api/health/setup').expect(200);
  ...
  describe('Service Dependencies', () => {
    ...
    it('should verify Ollama service', async () => {
      const response = await request(app).get('/api/health/ollama').expect(200);
