...
export default function NoteEditorPage() {
    ...
    async (saveData: { title: string; content: string }) => {
      ...
      try {
        ...
        const updatedNote = {
          ...
          ...saveData,
          updatedAt: new Date().toISOString(),
          wordCount: saveData.content.replace(/<[^>]*>/g, '').split(/\s+/).length,
          readingTimeMinutes: Math.ceil(saveData.content.replace(/<[^>]*>/g, '').split(/\s+/).length / 200)
        }
  ...
  const handleMetadataUpdate = useCallback(async (updates: Partial<Note>) => {
    if (!note) return
      ...
      <div className="container mx-auto px-4 py-6 max-w-6xl">
        <div className="flex gap-6">
          ...
          <aside className="w-80 space-y-6">
            ...
            {!saveStatus.error && !saveStatus.hasUnsavedChanges && saveStatus.lastSaved && (
              <Alert>
