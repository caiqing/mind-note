...
function verifyDatabaseSystem() {
  ...
  const files = [
    {
      name: '连接池管理器',
      path: 'src/lib/database/connection-pool-manager.ts',
    },
    { name: '查询监控器', path: 'src/lib/database/query-monitor.ts' },
    {
      name: '数据库监控演示',
      path: 'src/components/database/database-monitor-demo.tsx',
    },
  ];
  ...
  // 检查连接池管理器功能
  const poolManagerPath = path.join(
    __dirname,
  ...
  // 检查查询监控器功能
  const queryMonitorPath = path.join(
    __dirname,
  ...
  const demoPath = path.join(
    __dirname,
    'src/components/database/database-monitor-demo.tsx',
  );
  if (fs.existsSync(demoPath)) {
    ...
    const demoFeatures = [
      ...
      {
        name: '连接池状态卡片',
        pattern: /总连接数.*平均获取时间.*查询总数.*整体错误率/,
      },
      ...
      {
        name: '性能摘要卡片',
        pattern: /总查询数.*平均执行时间.*慢查询数.*错误率.*缓存命中率/,
      },
      ...
      {
        name: '状态颜色映射',
        pattern: /getPoolStatusColor.*getPerformanceLevelColor/,
      },
    ];
  ...
  console.log(
    '   📈 查询类型分布：SELECT(81%) INSERT(10%) UPDATE(6%) DELETE(3%)',
  );
