...
function verifyE2ESystem() {
  ...
  const files = [
    ...
    {
      name: '笔记管理测试',
      path: 'tests/e2e/scenarios/note-management.test.ts',
    },
  ...
  // 检查页面对象模型
  const pageObjectsPath = path.join(
    __dirname,
  ...
  const authFlowPath = path.join(
    __dirname,
    'tests/e2e/scenarios/auth-flow.test.ts',
  );
  ...
  // 检查笔记管理测试
  const noteManagementPath = path.join(
    __dirname,
    'tests/e2e/scenarios/note-management.test.ts',
  );
  ...
  // 检查AI功能测试
  const aiFeaturesPath = path.join(
    __dirname,
    'tests/e2e/scenarios/ai-features.test.ts',
  );
  ...
  console.log(
    '   🧪 运行测试套件: tsx tests/e2e/cli/e2e-cli.ts run-suite <suite>',
  );
  console.log(
    '   🎯 运行测试场景: tsx tests/e2e/cli/e2e-cli.ts run-scenario <scenario>',
  );
