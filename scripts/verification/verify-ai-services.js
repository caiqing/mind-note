...
verify(
  'openAIConfig',
  'OpenAIConfigManager类已定义',
  fileContains(
    'ai-services/cloud/openai/config.ts',
...
verify(
  ...
  'OpenAIProvider类已定义',
  fileContains(
    'ai-services/cloud/openai/openai-provider.ts',
...
verify(
  ...
  'GET方法支持',
  fileContains(
    'src/app/api/dev/ai/configure/route.ts',
...
console.log('\n📋 验证降级机制模块...');
// 降级机制验证
verify('fallback', '降级管理器文件存在', fileExists('src/lib/ai/fallback.ts'));
verify(
  'fallback',
  'AIFallbackManager类已定义',
...
console.log(
  `   📈 成功率: ${((totalPassed / (totalPassed + totalFailed)) * 100).toFixed(1)}%`,
);
