...
function verifyCacheSystem() {
  ...
  const files = [
    ...
    { name: '智能缓存策略', path: 'src/lib/cache/intelligent-cache-strategy.ts' },
    { name: '缓存性能监控', path: 'src/lib/cache/cache-monitor.ts' },
    { name: '缓存系统演示', path: 'src/components/cache/cache-system-demo.tsx' },
  ]
  ...
  // 检查多级缓存核心功能
  const multiLevelCachePath = path.join(__dirname, 'src/lib/cache/multi-level-cache.ts')
  ...
  // 检查智能缓存策略功能
  const strategyPath = path.join(__dirname, 'src/lib/cache/intelligent-cache-strategy.ts')
  if (fs.existsSync(strategyPath)) {
    ...
    const strategyFeatures = [
      ...
      { name: '智能策略管理器', pattern: /class IntelligentCacheStrategyManager/ },
      ...
      { name: '单例实例导出', pattern: /export const intelligentCacheStrategyManager/ },
    ]
  ...
  // 检查缓存系统演示功能
  const demoPath = path.join(__dirname, 'src/components/cache/cache-system-demo.tsx')
